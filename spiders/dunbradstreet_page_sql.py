    ...
    while True:
        ...
        for category_idx, category_data in enumerate(category_datas):
            ...
            for page_idx, page_data in enumerate(page_datas):
                ...
                print (f"{'Page':15s}:{str(page_data[1]):50s}:\t{(page_idx+1)*100/num_page_datas:.2f} %",end= '\r')
                ...
                if num_jobs >= limite:
                    ...
                    jobs, Q = do_jobs(DB_NAME, TB_NAME, i, category_idx, numCategorys, jobs, Q, page_idx, num_page_datas)
            categroy_parse += num_jobs
            jobs, Q = do_jobs(DB_NAME, TB_NAME, i, category_idx, numCategorys, jobs, Q, page_idx, num_page_datas)
        total += categorys
